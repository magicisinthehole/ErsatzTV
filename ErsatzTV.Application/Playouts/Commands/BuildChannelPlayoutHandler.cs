...
public class BuildChannelPlayoutHandler : IRequestHandler<BuildChannelPlayout, Either<BaseError, Unit>>
{
    ...
    public BuildChannelPlayoutHandler(
        IClient client,
    ...
    public async Task<Either<BaseError, Unit>> Handle(BuildChannelPlayout request, CancellationToken cancellationToken)
    {
    ...
    private async Task<Unit> ApplyUpdateRequest(TvContext dbContext, BuildChannelPlayout request, Playout playout)
    {
        try
        {
    ...
    private static Task<Validation<BaseError, Playout>> Validate(TvContext dbContext, BuildChannelPlayout request) =>
        PlayoutMustExist(dbContext, request);

    private static Task<Validation<BaseError, Playout>> PlayoutMustExist(
        TvContext dbContext,
        BuildChannelPlayout buildPlayout) =>
        dbContext.Playouts
            .Include(p => p.Channel)
            .ThenInclude(c => c.ChannelScheduleDayTemplates)
            .ThenInclude(t => t.ScheduleDayTemplate)
            .ThenInclude(t => t.ScheduleBlocks)
            .ThenInclude(sb => sb.Items)
            .Include(p => p.Items)
            ...
            .SelectOneAsync(p => p.ChannelId, p => p.ChannelId == buildPlayout.ChannelId)
